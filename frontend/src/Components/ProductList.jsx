...
function ProductList({ searchTerm }) {
  ...
  // Correctly accessing the Redux state
  const { items: products, loading, error } = useSelector(state => ({
    items: state.products.items, // Corrected this line
  ...
  console.log("Searched Products:", searchedProducts); // Debugging log

  return (
    <div className="product-list-container">
      <div className="product-grid">
        {searchedProducts.length > 0 ? (
          searchedProducts.map(product => { // Changed from products to product for clarity
            console.log("Product Data:", product); // Log each product data
            return (
              <ProductCard
