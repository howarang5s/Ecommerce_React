...
function ProductCard({ product, isAllProducts }) {
  ...
  // Check if product is valid, and use _id for validation
  if (!product || !product._id) {
    console.error("Invalid product data:", product); // Log the invalid product
    return <div className="error-message">Invalid product data</div>;
  }
  ...
  return (
    ...
    >
      ...
      <Link to={`/product/${product._id}`} className="product-detail-link">View Details</Link> {/* Use _id */}
    </div>
  );
}
